...
def inference(model):
    ...
    labels_actual = list(dataframe['Species'])
    ...
    for i in range(image_count):
        ...
        labels_predicted.append(y_pred[0])

    # Save inference times to csv
    with open(output_directory + "tf_inference_times.csv", 'w', newline='') as file:
        writer = csv.writer(file, delimiter=',')
        writer.writerow(['Filename', 'Preprocessing time (ms)', 'Inference time (ms)', 'Predicted label'])
        for i in range(image_count):
            writer.writerow([filenames[i], preprocessing_times[i] * 1000, inference_times[i] * 1000, labels_predicted[i]])
