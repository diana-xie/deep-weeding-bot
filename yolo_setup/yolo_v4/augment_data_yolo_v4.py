...
def prepare_images(labels_raw_path: str,
                   images_raw_path: str,
                   images_train_path: str,
                   height: int,
                   width: int):
    """
    :param labels_raw_path: path of folder containing original xml labels for images
    :param images_raw_path: path of folder containing raw images
    :param images_train_path: path of folder containing images that were labelled for training + later augmented images
    ...
    labels_df = xml_to_csv(os.path.dirname(labels_raw_path))
    labels_df.to_csv('labels.csv', index=False)
    print('Successfully converted xml to csv.')

    # Copy images that were labelled
    files = glob.glob(labels_raw_path + '/*.xml')
    files = [re.findall(r'\d+', x)[0] for x in files]
    files = [images_raw_path + '/frame' + str(x) + '.jpg' for x in files]
    for file in files:
        shutil.copy(file, images_train_path)  # file, destination
    ...
    resized_images_df = resize_imgaug(df=labels_df,
                                      images_path=images_train_path,
                                      aug_images_path=images_train_path, # source & aug folders same
                                      image_prefix='',
                                      height=height,
                                      width=width)
    ...
    group_df = group_df.drop(['index'], axis=1)
    bb_array = group_df.drop(['filename', 'width', 'height', 'class'], axis=1).values
    image = imageio.imread('images_train/frame100.jpg')
    bbs = BoundingBoxesOnImage.from_xyxy_array(bb_array, shape=image.shape)
...
def augment_images(use_prepare_images: bool = False,
                   ...
                   width: int = None):
    """
    :param use_prepare_images: if True, apply the function to convert all XML files in images/ folder into labels.csv
    :param height: height of resized image
    :param width: width of resized image
    ...
    if use_prepare_images:
        labels_df, resized_images_df = prepare_images(
            labels_raw_path=os.path.dirname(os.path.dirname(__file__)) + '/labels_raw/',
            images_raw_path=os.path.dirname(os.path.dirname(__file__)) + '/images_raw/',
            images_train_path='images_train/',
            height=height,
            width=width
        )
    else:
    ...
    # Apply augmentation to our images and save files into 'images_aug/' folder with 'aug1_' prefix.
    # Write the updated images and bounding boxes annotations to the augmented_images_df dataframe.
    augmented_images_df = image_aug(df=resized_images_df,
    ...
    for file in os.listdir('images_aug'):
        shutil.copy('images_aug/' + file, 'images_train/' + file)
...
def csv_to_xml(csv_file: str,
    ...
    :param labels_train_path: path of folder where output xml's will be saved
    :return: none
    """
    # get data
    ...
    for filename in filenames:
        ...
        tree.write(labels_train_path + filename.replace('.jpg', '.xml'), encoding='utf8')
...
def main():
    ...
    csv_to_xml(csv_file='all_labels.csv',
